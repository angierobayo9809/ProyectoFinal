<?php 
    ...
    foreach($result->fetchAll(PDO::FETCH_OBJ) as $r){
        $mat_id = $r->id;
        $mat_nombre = $r->nombre;
        $mat_notaMin = $r->notaMin;
        $mat_notaMax = $r->notaMax;
        $mat_esElectiva = $r-> esElectiva == 1 ? "checked" : "";
    }
                    ...
                    <div class="card-body">
                        <form action="update.php" method="POST" name="forrol">
                            ...
                            <div class="form-group">
                                ...
                                <input type="number" class="form-control" id="notaMin" name="notaMin" value="<?= $mat_notaMin ?>"  required>
                            </div>
                            <div class="form-group">
                                <label for="nombre">Nota máxima</label>
                                <input type="number" class="form-control" id="notaMax" name="notaMax" value="<?= $mat_notaMax ?>"  required>
                            </div>
                            <div class="form-group">
                                <label for="nombre">Es electiva</label>
                                <input type="checkbox" class="form-control" id="esElectiva" name="esElectiva" <?= $mat_esElectiva ?>>
                            </div>
